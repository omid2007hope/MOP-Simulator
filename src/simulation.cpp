                                              ...
                                              double dt)
{
    ...
    if (current_altitude > 0.0) {
        std::cout << "\n--- Simulating Atmospheric Drop from " << current_altitude << " ft ---\n";
        double dt_drop = 0.01;
        ...
        while (current_altitude > 0.0) {
            auto calc_derivs = [&](double alt_m, double vx, double vy) -> DropDeriv {
                ...
                double cd = EnvironmentPhysics::getMachDependentDrag(mach, dragCoefficient, proj, cons);
            ...
            if (current_altitude <= next_print_altitude && current_altitude > 0.0) {
                double cd = EnvironmentPhysics::getMachDependentDrag(current_velocity / current_atm.speed_of_sound_ms, dragCoefficient, proj, cons);
                double drag_force = 0.5 * current_atm.density_kgm3 * std::pow(current_velocity, 2) * cd * area;
                                                ...
                                                double dt)
{
    ...
    while (current_velocity > 0.0 && !res.casing_failure && current_depth < fullDepth) {
        ...
        double layerEntryDepth = (current_layer_idx == 0) ? 0.0 : layer_bottom_depths[current_layer_idx - 1];

        double squaredVelocity = current_velocity * current_velocity;
        double baseStrength = layer.compressive_strength + (layer.rebar_yield_strength * layer.rebar_volume_fraction);
        ...
        if (!erosion_active && proj.yield_strength > 0.0 && dynamic_pressure >= proj.yield_strength) {
            erosion_active = true;
            res.erosion_occurred = true;
            std::cout << "  [WAPM EROSION ONSET] Hydrodynamic pressure exceeded casing yield at Depth: "
        ...
        if (obliquity_radians > 0.0 || angleOfAttack_radians > 0.0) {
            ...
            if (proj.area_moment_inertia > 0) {
                max_bending_stress = (bending_moment * (proj.diameter / 2.0)) / proj.area_moment_inertia;
            }
        ...
        auto derivative = [&](double v, double z, double theta, double T, double L, double m) -> PenDeriv {
            ...
            if (theta > 0.0 || angleOfAttack_radians > 0.0) {
                lateral_force = (0.5 * baseDensity * vSq * area) * std::sin(theta + angleOfAttack_radians);
            }
            ...
            if (!erosion_active) {
                ...
                double axialForce = (craterDepthLimit > 0.0 && zLocal < craterDepthLimit)
                ...
                double u = EnvironmentPhysics::solveInterfaceVelocity(v, proj.casing_density, baseDensity, proj.yield_strength, effective_strength);
                double Le = std::max(0.01, L);
                d.dv = -(proj.yield_strength / (proj.casing_density * Le)) * (1.0 + (v - u) / bar_wave_speed) + gravity_component;
        ...
        PenDeriv k1 = derivative(current_velocity, current_depth, obliquity_radians, current_temperature, current_length, get_mass(current_length));
        PenDeriv k2 = derivative(current_velocity + 0.5 * dt * k1.dv, current_depth + 0.5 * dt * k1.dz, obliquity_radians + 0.5 * dt * k1.dtheta, current_temperature + 0.5 * dt * k1.dT, current_length + 0.5 * dt * k1.dL, get_mass(current_length + 0.5 * dt * k1.dL));
        PenDeriv k3 = derivative(current_velocity + 0.5 * dt * k2.dv, current_depth + 0.5 * dt * k2.dz, obliquity_radians + 0.5 * dt * k2.dtheta, current_temperature + 0.5 * dt * k2.dT, current_length + 0.5 * dt * k2.dL, get_mass(current_length + 0.5 * dt * k2.dL));
        PenDeriv k4 = derivative(current_velocity + dt * k3.dv, current_depth + dt * k3.dz, obliquity_radians + dt * k3.dtheta, current_temperature + dt * k3.dT, current_length + dt * k3.dL, get_mass(current_length + dt * k3.dL));
        ...
        if (!erosion_active) {
            ...
            if (current_length < 0.05 * proj.length) {
                ...
                res.outcome_summary = "Projectile fully eroded by hydrodynamic penetration.";
                break;
            }
        }
    ...
    if (res.is_kinetic_rod) {
        ...
        if (res.casing_failure) {
            ...
            res.shock_damage_prob_percent = std::min(100.0, 100.0 * max_walker_wasley_product / criticalEnergy);
            ...
            if (shockInitiates) {
                ...
                res.outcome_summary = "Transmitted shock exceeded the Walker-Wasley critical initiation energy (P^2*tau >= Ec).";
            } else {
                res.explosive_charge_survives = true;
                if (res.erosion_occurred) {
                    res.regime = "Hypervelocity Erosion (Walker-Anderson)";
...
SimulationResult ImpactSimulator::simulate(const ImpactScenario& scenario)
{
    ...
    res.mach_number = scenario.velocity / EnvironmentPhysics::standardAtmosphere(scenario.altitude_ft / 3.28084, cons).speed_of_sound_ms;
