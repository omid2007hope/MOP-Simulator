...
T getValidInput(const std::string& prompt, T defaultValue, bool allowZero = false) {
    ...
    while (true) {
        std::cout << prompt;
        if (std::cin >> val && (val > 0 || (allowZero && val == 0))) {
            return val;
        }
        std::cin.clear();
...
{
    std::cout << "===================================================================================================\n";
    std::cout << "                 C++ IMPACT PHYSICS & PENETRATION SIMULATOR (WITH 3D VISUALIZATION)                \n";
    std::cout << "===================================================================================================\n";
    ...
    if (choice == 2) {
        ...
        mop.length = getValidInput<double>("Enter Projectile Length L (meters) [default 6.2]: ", 6.2);
        mop.diameter = getValidInput<double>("Enter Projectile Diameter d (meters) [default 0.8]: ", 0.8);
        mop.total_mass = getValidInput<double>("Enter Total Mass m (kg) [default 13600]: ", 13600.0);
        mop.explosive_mass = getValidInput<double>("Enter Explosive Mass (kg) [default 2400, enter 0 for kinetic rod]: ", 2400.0, true);
        mop.casing_density = getValidInput<double>("Enter Casing Density rho_p (kg/m^3) [default 7800 for steel, 19300 for tungsten]: ", 7800.0);
        double yield = getValidInput<double>("Enter Casing Yield Strength sigma_y (GPa) [default 2.0, enter 0 for kinetic rod]: ", 2.0, true);
        mop.yield_strength = yield * 1e9;
        concrete.density = getValidInput<double>("Enter Target Concrete Density rho_t (kg/m^3) [default 2500]: ", 2500.0);

        int numScenarios = 1;
        while (true) {
            std::cout << "Enter number of custom impact velocities to test [1 to 10]: ";
        ...
        for (int i = 0; i < numScenarios; ++i) {
            ...
            prompt_ss << "  -> Enter Velocity #" << (i + 1) << " (m/s) [e.g., 3500, 2000, 350]: ";
            double v = getValidInput<double>(prompt_ss.str(), 1000.0);
            
            std::stringstream name_ss;
            name_ss << "Custom Test #" << (i + 1) << " (" << v << " m/s)";
            scenarios.push_back({name_ss.str(), 0.0, v});
        }
