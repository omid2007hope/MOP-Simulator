...
int main() {
    ...
    if (choice == 2) {
        ...
        std::string pName;
        std::cin >> pName;
        if (!pName.empty()) mop.name = pName;

        std::cout << "Enter Projectile Length L (meters) [default 6.2]: ";
        double val;
        if (std::cin >> val && val > 0) mop.length = val; else clearCinBuffer();

        std::cout << "Enter Projectile Diameter d (meters) [default 0.8]: ";
        if (std::cin >> val && val > 0) mop.diameter = val; else clearCinBuffer();
        ...
        for (int i = 0; i < numScenarios; ++i) {
            std::cout << "  -> Enter Velocity #" << (i+1) << " (m/s) [e.g., 3500, 2000, 350]: ";
            double v = 1000.0;
            if (std::cin >> v && v > 0) {
                std::stringstream ss;
