...
int main(int argc, char* argv[]) {
	...
	// ==========================================
		...
		if (choice == 1) {
			...
			if (auto p = ConfigLoader::getProjectileByName(
				    projectilesDb,
				    "Orbital Tungsten Kinetic Penetrator (Rods from God)")) {
				munition = *p;
			} else {
			...
			std::cout
				<< "  [1] 2-Bomb Lead Strike (Initial Overburden Shaft Breaker + Follow-on Strike)\n";
			std::cout
				<< "  [2] 4-Bomb Shaft Sequence (Fordow Centrifuge Vault Destruction)\n";
			std::cout
				<< "  [3] 6-Bomb Maximum Salvo Strike (Maximum Subterranean Overpressure Wave)\n";
			int hammerChoice = 1;
			while (true) {
				std::cout << "Selection (1-3): ";
				if (std::cin >> hammerChoice && hammerChoice >= 1 &&
				    hammerChoice <= 3) {
					safeCin();
					break;
				}
