...
- **The Flaw**: In the interactive CLI mode (Choice 2), there is a severe bug regarding the projectile name:
  ...
  ```
  First, this uses `!projectileName.empty()` which means if the user _does_ type a name, it gets overwritten with "Undefined Projectile". It should be `if (projectileName.empty())`.
