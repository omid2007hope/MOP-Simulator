...
\begin{equation}
\tau_p = \frac{2 t_w}{U_s} \approx 37.2\text{ \mu s}
\end{equation}
...
Across 100.0\% of the simulated scenarios, the peak contact pressure substantially exceeded the explosive's critical Hugoniot initiation threshold. Within $\tau_p = 37.2\text{ \mu s}$, the intense stress wave propagating through the casing wall converted shock energy into local hot-spot heating within the explosive matrix. This induced immediate Shock-to-Detonation Transition (SDT).

Because explosive detonation occurred at $t = 37.2\text{ \mu s}$ following boundary touch, the energetic fill reacted fully while the nose of the weapon was still situated at the physical interface ($z = 0.00\text{ m}$). The casing suffered catastrophic structural fragmentation ($100.0\%\text{ casing failure}$). 
...
\item \textbf{Casing Disruption:} The energetic payload detonated within a $37.2\text{ \mu s}$ pulse window, destroying the steel casing prior to target displacement.
